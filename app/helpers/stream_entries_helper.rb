...
module StreamEntriesHelper
  ...
  def account_description(account)
    return "The latest Posts from #{display_name(account)} (@#{account.username}). #{account.note}"
  end

  def group_description(group)
    return "#{group.title} on Vegeterians.live. #{group.description}"
  end
  ...
  def status_description(status, account)
    ...
    return "#{display_name(account)} (@#{account.username}) on Vegeterians.live. #{components.reject(&:blank?).join("\n\n")}"
  end
