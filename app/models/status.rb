...
class Status < ApplicationRecord
  ...
  scope :popular_accounts, -> { left_outer_joins(:account).where('accounts.is_verified=true OR accounts.is_pro=true') }
  scope :pro_accounts, -> { left_outer_joins(:account).where('accounts.is_pro=true') }
  scope :expert_accounts, -> { left_outer_joins(:account).where('accounts.is_expert=true') }
  scope :trends_account, -> { left_outer_joins(:account).where('accounts.username = ?', "Vegeteriansnews") }
  ...
  class << self
    ...
    def as_pro_timeline(account = nil)
      query = timeline_scope.without_replies.pro_accounts
      apply_timeline_filters(query, account)
    end

    def as_expert_timeline(account = nil)
      query = timeline_scope.without_replies.expert_accounts
      apply_timeline_filters(query, account)
    end
    ...
    def as_trends_timeline()
      timeline_scope.without_replies.trends_account
    end
