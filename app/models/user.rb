...
#  unique_email              :string
#  promoter_auth_token       :string
#  available_boost_post      :integer
#
...
class User < ApplicationRecord
  ...
  def check_membership_pro_plus_user
    @subscription_data = self.account.membership_subscription
    ...
    @pro_monthly = Stripe::Price.retrieve("ENV['MONTHLY_PRO_PLAN_ID']").unit_amount/100 rescue nil
    @pro_yearly = Stripe::Price.retrieve(ENV['YEARLY_PRO_PLAN_ID']).unit_amount/100 rescue nil
    @proplus_monthly = Stripe::Price.retrieve(ENV['MONTHLY_PRO_PLUS_PLAN_ID']).unit_amount/100 rescue nil
    @proplus_yearly = Stripe::Price.retrieve(ENV['YEARLY_PRO_PLUS_PLAN_ID']).unit_amount/100 rescue nil
  ...
  # allow username or email address for login (Vegeterians.live)
