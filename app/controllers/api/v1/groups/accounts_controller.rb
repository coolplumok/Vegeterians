...
class Api::V1::Groups::AccountsController < Api::BaseController
  ...
  def show
    ...
    render json: @accounts, each_serializer: REST::GroupAccountSerializer, relationships: GroupMemberRelationshipsPresenter.new(@accounts, @group.id)
  end
  ...
  def update
    ...
    @account = @group.accounts.includes(:account_stat).find(params[:account_id])
    render json: @account, serializer: REST::GroupAccountSerializer, relationships: GroupMemberRelationshipsPresenter.new([@account.id], @group.id)
  end
  ...
  def load_accounts
    if unlimited?
      @group.accounts.includes(:account_stat).order(role: :asc).all
    else
      @group.accounts.includes(:account_stat).order(role: :asc).paginate_by_max_id(limit_param(DEFAULT_ACCOUNTS_LIMIT), params[:max_id], params[:since_id])
    end
  end
