...
module Admin
  class AccountsController < BaseController
    before_action :set_account, only: [:show, :subscribe, :unsubscribe, :redownload, :remove_avatar, :remove_header, :enable, :unsilence, :unsuspend, :memorialize, :approve, :reject, :verify, :unverify, :add_proplus_badge, :remove_proplus_badge, :add_expert_badge, :remove_expert_badge, :edit_pro, :save_pro, :edit, :update]
    ...
    def add_proplus_badge
      authorize @account, :update_badges?

      @account.is_proplus = true
      @account.save!

      log_action :add_proplus_badge, @account.user
      redirect_to admin_account_path(@account.id)
    end

    def remove_proplus_badge
      authorize @account, :update_badges?

      @account.is_proplus = false
      @account.save!

      log_action :remove_proplus_badge, @account.user
      redirect_to admin_account_path(@account.id)
    end

    def add_expert_badge
      authorize @account, :update_badges?

      @account.is_expert = true
      @account.save!

      log_action :add_expert_badge, @account.user
      redirect_to admin_account_path(@account.id)
    end

    def remove_expert_badge
      authorize @account, :update_badges?

      @account.is_expert = false
      @account.save!

      log_action :remove_expert_badge, @account.user
      redirect_to admin_account_path(@account.id)
    end
