...
class OStatus::AtomSerializer
  ...
  def group_invite_salmon(group_invite)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    description = "#{group_invite.account.acct} started group invite #{group_invite.target_account.acct}"

    append_element(entry, 'id', OStatus::TagManager.instance.unique_tag(group_invite.created_at, group_invite.id, 'GroupInvite'))
    append_element(entry, 'title', description)
    append_element(entry, 'content', description, type: :html)

    entry << author(group_invite.account)

    append_element(entry, 'activity:object-type', OStatus::TagManager::TYPES[:activity])
    append_element(entry, 'activity:verb', OStatus::TagManager::VERBS[:group_invite])

    object = author(group_invite.target_account)
    object.value = 'activity:object'
  ...
  def group_invite_request_salmon(group_invite_request)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    append_element(entry, 'id', OStatus::TagManager.instance.unique_tag(group_invite_request.created_at, group_invite_request.id, 'GroupInviteRequest'))
    append_element(entry, 'title', "#{group_invite_request.account.acct} requested to invite #{group_invite_request.target_account.acct} on #{group_invite_request.group.name}")

    entry << author(group_invite_request.account)

    append_element(entry, 'activity:object-type', OStatus::TagManager::TYPES[:activity])
    append_element(entry, 'activity:verb', OStatus::TagManager::VERBS[:request_group_invite])

    object = author(group_invite_request.target_account)
    object.value = 'activity:object'
  ...
  def group_join_private_salmon(group_join)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    description = "#{group_join.account.acct} started group join #{group_join.target_account.acct}"

    append_element(entry, 'id', OStatus::TagManager.instance.unique_tag(group_join.created_at, group_join.id, 'GroupJoinPrivate'))
    append_element(entry, 'title', description)
    append_element(entry, 'content', description, type: :html)

    entry << author(group_join.account)

    append_element(entry, 'activity:object-type', OStatus::TagManager::TYPES[:activity])
    append_element(entry, 'activity:verb', OStatus::TagManager::VERBS[:group_join])

    object = author(group_join.target_account)
    object.value = 'activity:object'
  ...
  def group_join_private_request_salmon(group_join_request)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    append_element(entry, 'id', OStatus::TagManager.instance.unique_tag(group_join_request.created_at, group_join_request.id, 'GroupJoinPrivateRequest'))
    append_element(entry, 'title', "#{group_join_request.account.acct} requested to join on #{group_join_request.group.name}")

    entry << author(group_join_request.account)

    append_element(entry, 'activity:object-type', OStatus::TagManager::TYPES[:activity])
    append_element(entry, 'activity:verb', OStatus::TagManager::VERBS[:request_group_join])

    object = author(group_join_request.target_account)
    object.value = 'activity:object'
  ...
  def group_approve_join_private_salmon(group_join)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    description = "#{group_join.account.acct} approved group join #{group_join.target_account.acct}"
  ...
  def group_approve_join_private_request_salmon(group_join_request)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    append_element(entry, 'id', OStatus::TagManager.instance.unique_tag(group_join_request.created_at, group_join_request.id, 'GroupJoinPrivateRequest'))
    append_element(entry, 'title', "#{group_join_request.account.acct} approved to join on #{group_join_request.group.name}")
  ...
  def health_expert_application_salmon(health_expert_application)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    description = "#{health_expert_application.account.acct} applied as the Health Expert"

    append_element(entry, 'id', OStatus::TagManager.instance.unique_tag(health_expert_application.created_at, health_expert_application.id, 'HealthExpertApplication'))
    append_element(entry, 'title', description)
    append_element(entry, 'content', description, type: :html)

    entry << author(health_expert_application.account)

    append_element(entry, 'activity:object-type', OStatus::TagManager::TYPES[:activity])
    append_element(entry, 'activity:verb', OStatus::TagManager::VERBS[:health_expert_application])

    object = author(health_expert_application.account)
    object.value = 'activity:object'
  ...
  def health_expert_application_request_salmon(health_expert_application_request)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    append_element(entry, 'id', OStatus::TagManager.instance.unique_tag(health_expert_application_request.created_at, health_expert_application_request.id, 'HealthExpertApplication'))
    append_element(entry, 'title', "#{health_expert_application_request.account.acct} applied as the Health Expert")

    entry << author(health_expert_application_request.account)

    append_element(entry, 'activity:object-type', OStatus::TagManager::TYPES[:activity])
    append_element(entry, 'activity:verb', OStatus::TagManager::VERBS[:request_health_expert_application])

    object = author(health_expert_application_request.target_account)
    object.value = 'activity:object'
  ...
  def health_expert_approve_salmon(health_expert_approve)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    description = "#{health_expert_approve.account.acct} approved the Health Expert Application #{health_expert_approve.target_account.acct}"

    append_element(entry, 'id', OStatus::TagManager.instance.unique_tag(health_expert_approve.created_at, health_expert_approve.id, 'HealthExpertApprove'))
    append_element(entry, 'title', description)
    append_element(entry, 'content', description, type: :html)

    entry << author(health_expert_approve.account)

    append_element(entry, 'activity:object-type', OStatus::TagManager::TYPES[:activity])
    append_element(entry, 'activity:verb', OStatus::TagManager::VERBS[:health_expert_approve])

    object = author(health_expert_approve.target_account)
    object.value = 'activity:object'
  ...
  def health_expert_approve_request_salmon(health_expert_approve_request)
    entry = Ox::Element.new('entry')
    add_namespaces(entry)

    append_element(entry, 'id', OStatus::TagManager.instance.unique_tag(health_expert_approve_request.created_at, health_expert_approve_request.id, 'HealthExpertApprove'))
    append_element(entry, 'title', "#{health_expert_approve_request.account.acct} approved the Health Expert Application ")

    entry << author(health_expert_approve_request.account)

    append_element(entry, 'activity:object-type', OStatus::TagManager::TYPES[:activity])
    append_element(entry, 'activity:verb', OStatus::TagManager::VERBS[:request_health_expert_approve])

    object = author(health_expert_approve_request.target_account)
    object.value = 'activity:object'
