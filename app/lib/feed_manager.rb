...
class FeedManager
  ...
  def filter?(timeline_type, status, receiver_id)
    ...
    elsif timeline_type == :common
      filter_from_common?(status, receiver_id)
    else
      false
    end
  end
  ...
  def filter_from_home?(status, receiver_id)
    ...
    if status.group
      if status.group.is_privated
        if !status.group.group_accounts.where(account_id: receiver_id).exists?
          return true
        end
      end
    end
  ...
  def filter_from_common?(status, receiver_id)
    return false if receiver_id == status.account_id
  ...
  def filter_from_mentions?(status, receiver_id)
    ...
    # necessary to check the author of the post for mute/block again
    check_for_blocks = status.active_mentions.pluck(:account_id)
