...
class PotentialGroupInviteTracker
  ...
  class << self
    ...
    def record(account_id, target_account_id, group_id, action)
      ...
      key    = "groupinvites:#{account_id}group:#{group_id}"
    ...
    def remove(account_id, target_account_id, group_id)
      redis.zrem("groupinvites:#{account_id}group:#{group_id}", target_account_id)
    end

    def get(account_id, group_id, limit: 10, offset: 0)
      account_ids = redis.zrevrange("groupinvites:#{account_id}group:#{group_id}", offset, limit)
