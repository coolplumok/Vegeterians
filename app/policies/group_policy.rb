...
class GroupPolicy < ApplicationPolicy
  ...
  def create?
    admin? or current_account.is_pro || current_account.is_proplus
  end
  ...
  def leave?
    ...
    is_account_the_only_admin = (is_group_admin_or_moderator? and record.group_accounts.where(role: :admin).count == 1)
  ...
  def is_group_admin_or_moderator?
    record.group_accounts.where(account_id: current_account.id, role: :moderator).exists?
  end
