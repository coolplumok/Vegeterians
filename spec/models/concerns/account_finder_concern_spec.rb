...
describe AccountFinderConcern do
  ...
  describe 'remote finders' do
    before do
      @account = Fabricate(:account, username: 'Alice', domain: 'Vegeterians.live')
    end

    describe '.find_remote' do
      it 'returns exact match result' do
        expect(Account.find_remote('alice', 'Vegeterians.live')).to eq(@account)
      end
      ...
      it 'returns nil when username does not match' do
        expect(Account.find_remote('a_ice', 'Vegeterians.live')).to be_nil
      end
