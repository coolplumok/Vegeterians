...
namespace :gabsocial do
  ...
  task :setup do
    ...
    begin
      ...
      using_docker        = prompt.yes?('Are you using Docker to run Vegeterians.live?')
      ...
      loop do
        ...
        env['SMTP_FROM_ADDRESS'] = prompt.ask('E-mail address to send e-mails "from":') do |q|
          q.required true
          q.default "Vegeterians.live <noreply@#{env['LOCAL_DOMAIN']}>"
          q.modify :strip
        end
        ...
        begin
          ...
          mail = ActionMailer::Base.new.mail to: send_to, subject: 'Test', body: 'Vegeterians.live SMTP configuration works!'
          mail.deliver
          break
        rescue StandardError => e
      ...
      if prompt.yes?('Save configuration?')
        ...
        prompt.say "\n"
        prompt.ok 'All done! You can now power on the Vegeterians.live server.'
        prompt.say "\n"
